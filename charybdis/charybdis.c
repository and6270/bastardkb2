...
static void maybe_update_pointing_device_cpi(charybdis_config_t* config) {
    ...
    } else if (config->is_carret_enabled) {
        pointing_device_set_cpi(CHARYBDIS_CARRET_BUFFER);    
    } else {
...
void tap_tb(uint8_t keycode0, uint8_t keycode1, uint8_t keycode2, uint8_t keycode3, int16_t *move_buffer_x, int16_t *move_buffer_y) {
    uint16_t local_carret_dpi = g_charybdis_config.is_integ_enabled? CHARYBDIS_CARRET_BUFFER * 8 : CHARYBDIS_CARRET_BUFFER;
